...
*Why:* considered re-checking `/info` at the start of every session too, defensively. Dropped it — the first `GetItems` poll after `S` already surfaces a disconnected reader/neutralizer through the same mid-poll handling below, at most one poll interval later. A separate recheck would just duplicate that.
...
- **`outcome` is nullable** — covers both an aborted session (`Ctrl+C` mid-session) and a closed session with an empty basket.
...
- **Transaction number is timestamp-based, not a UUID** — sortable, more compact, and collision-free by construction since sessions can't overlap (point 2).
...
| App        | `0`               | None                               | Success                                                                                                                                                                                                                            | Determined**only** by `ErrorCode` — `ErrorMessage` can be non-null even on success (the "no items in the tub" example), so it's never used to infer failure.                                        |
...
- **Retry scope depends on what's actually known**: full resend when nothing was touched (`4`/`8`) or when genuinely unsure (connection failure, `1`); a scoped resend of just the failed subset for `17`, the one case with enough information to avoid re-touching successes.
